...
select   *,
         ...
         case when strpos(step_01_remove_mon_fri, ':') > 0
              then substr(step_01_remove_mon_fri, 1, strpos(step_01_remove_mon_fri, ':') - 1)
              else null
...
select   *,
         case when step_05_times_prefix is not null
              then trim(replace(step_01_remove_mon_fri, concat(step_05_times_prefix, ':'), ''))
